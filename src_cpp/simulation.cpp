...
/*
    ...
    array<array<int, L>, Nmax> V;
    V = loadconffile("V", 1.0*Dt/Dx);
    
    
    array<array<array<array<array<int, L>, Nmax>, nkind>, nordest>, ndir> lanes, LC, RC, EL;
    ...
    if (valid==true){ // if the configuration is valid
        ...
        for (int TIME=0; TIME<3*3600;TIME++){      
            ...
            buschangelane(BusesPar, BusesBool,SYSTEM, LC, RC, EL);
            calculategaps(BusesPar,BusesBool, EL, LC, RC);
    ...
    string text = "";
    for (auto line: SYSTEM.Lines){
        //std::cout<<line.name<<" "<<line.biart<<std::endl;
        text = text + line.name +",";
    ...
    // exporting the headways and dwell times for each
    for (auto line: SYSTEM.Lines){
        filename = fileroot + "_headway_dwell_"+line.name+".txt";
        text = "";
        for (int i=0; i< line.dwells.size(); i++){
            text = text + to_string(line.dts[i])+","+to_string(line.dwells[i])+"\n";
        }
        ofstream timesfile;
