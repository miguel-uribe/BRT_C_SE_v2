...
for dir in [0,1]:
    ...
    for dest in [0,1]:
        ...
        for door in np.arange(7):
            ...
            if (dir==1) & (dest==0):
                LCR.append(['LNA', 1, begS+DLBS+DLLS-Db[1]-2*cld, endL])
            ...
            # Also to the north, the stopping lane is not available before reaching the docking bay
            #if (door>1) & (dir ==0) & (dest==0) & (door<6):
            #    LCR.append(['LNA', 1, begS, begS + wN[dir,door]-2*Db[1]-2*cld])   
            ...
            # also to the south, the stoppimg lane is not available before reaching the docking lane
            #if (door>1) & (dir==1) & (door<6):
            #    LCR.append(['LNA', 1, begS, begS + wN[dir,door]-2*Db[1]-2*cld])   
...
# beginning, end, origin/destination, direction, door

serv_dict = {   'B27':  [bL   ,L+rbL  ,0 , 0 , 5],
                '8N':   [bL   ,L+rbL  ,0 ,0 , 4],
                'B75':  [bL   ,L+rbL  ,0 ,0 , 3],
                'B26':  [bL   ,L+rbL  ,0 ,0 , 3],
                'B10':  [bL   ,L+rbL  ,0 ,0 , 3],
                'B12':  [bL   ,L+rbL  ,1 ,0 , 1],
                'B46':  [bL   ,L+rbL  ,1 ,0 , 1],
                'G12':  [bL+L ,2*L+rbL,1 ,1, 5],
                'G46':  [bL+L ,2*L+rbL,1 ,1, 5],
                '8S':   [bL+L ,2*L+rbL,0 ,1, 3],
                'H27':  [bL+L ,2*L+rbL,0 ,1, 3],
                'F26':  [bL+L ,2*L+rbL,0 ,1, 3],
                'D10':  [bL+L ,2*L+rbL,0 ,1, 1],
                'H75':  [bL+L ,2*L+rbL,0 ,1, 1],
                'GenS_NQS': [bL+L ,2*L+rbL,1 ],
                'GenN_NQS': [bL   ,L+rbL,1 ],
                'GenS_Car': [bL+L ,2*L+rbL,0 ],
                'GenN_Car': [bL   ,L+rbL,0 ]
             }
...
for service_name, stops in serv_dict.items():
    ...
    try: # if the service is stop
        text = text + "%d %d %d %d %d %d"%(serviceID,stops[0], stops[1], stops[2], stops[3], stops[4])
        text = text + "\n"
    except: # if the service does not stop
        text = text + "%d %d %d %d"%(serviceID,stops[0], stops[1], stops[2])
        text = text + "\n"
dirname = os.path.dirname(__file__)
...
# lineID - headway - base_dwell_time - headway_dwelltime - base_variance - headway_variance - biart_fraction
dirname = os.path.dirname(__file__)
...
for i,_ in enumerate(serv_dict.keys()):
    text =text + "%d %d %d %f %f %f %f\n"%(i,headways[i], dwelltimes[i], dwelltimes2[i], variance[i], variance2[i], biart[i])
